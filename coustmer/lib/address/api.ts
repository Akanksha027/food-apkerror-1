...
export const addressApi = {
  ...
  geocode: async (input: { placeId?: string; address?: string }): Promise<GeocodeResult> => {
    try {
      ...
      const lat = Number((location.lat ?? (location.location as { lat?: number })?.lat) as unknown);
      const lng = Number((location.lng ?? (location.location as { lng?: number })?.lng) as unknown);
